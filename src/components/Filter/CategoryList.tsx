  ...
  const handleCategoryCheckboxChange = (
    ...
    if (e.target.checked) {
      setIngredientsToFilter((prev: string[]) => [
        ...prev.filter((ingredient: string) => !category.ingredients.includes(ingredient)),
  ...
  const handleDetailViewButtonPress = (category: Category) => {
    ...
    if (expandedCategories.includes(category.name)) {
      setExpandedCategories((prev: string[]) =>
        [...prev].filter((categoryName) => categoryName !== category.name),
      );
    }
    // expand
    else {
              ...
              return (
                <li key={cat.name}>
                  <input
                    ...
                    id={cat.name}
                    name={cat.name}
                    checked={cat.ingredients.every((catIngredient) =>
                      ingredientsToFilter.includes(catIngredient),
                    )}
